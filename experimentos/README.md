...
| `gerar_resultados.py` | Reúne tempos, ambiente e gráficos em `resultados.md` para leitura e análise. |
...
7. Gere o relatório de leitura da rodada local:

```bash
python3 experimentos/gerar_resultados.py
```

O arquivo [resultados.md](resultados.md) organiza as configurações, medianas, speedup, eficiência, verificação do fitness, registro de ambiente e gráficos. Ele aponta para as evidências originais e deve ser regenerado após cada nova coleta.

8. Consulte `dados/resumo.csv` e os gráficos em `graficos/`. As métricas principais são:
