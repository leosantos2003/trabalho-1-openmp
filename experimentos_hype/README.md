...
| `gerar_resultados_hype.py` | Reúne tempos, ambiente, gráficos e VTune em `resultados_hype.md`. |
| `analise_resultados_hype.md` | Análise crítica da rodada coletada e das limitações que afetam sua validade. |
| `GUIA_ACESSO_E_EXECUCAO.md` | Roteiro prático de SSH, Slurm, calibração, coleta e cópia dos resultados. |
...
Para executar a coleta no PCAD/Hype pela primeira vez ou retomá-la depois de interrompê-la, siga [GUIA_ACESSO_E_EXECUCAO.md](GUIA_ACESSO_E_EXECUCAO.md). Ele contém os comandos de SSH, `rsync`, `salloc`, acompanhamento da fila e cancelamento.
...
7. Após a coleta do VTune (ou antes, para gerar um relatório parcial), consolide as evidências:

```bash
python3 experimentos_hype/gerar_resultados_hype.py
```

O arquivo [resultados_hype.md](resultados_hype.md) reúne configurações, medianas, validação de fitness, ambiente, gráficos e um índice dos relatórios do VTune. Gere-o novamente sempre que acrescentar dados ou resultados do profiler.

8. Revise `dados/resumo.csv`. Para cada conjunto, verifique se `scaling_speedup` começa em 1, se `scaling_efficiency` começa em 1 e se as amostras possuem contagem igual a `REPETITIONS`.

9. Selecione uma configuração interessante, normalmente um conjunto médio/grande com eficiência menor que a esperada, e execute o VTune conforme [vtune/README.md](vtune/README.md).
